  ...
  <div class="cart">
    ...
    <div class="cart-tool">
      ...
      <div class="money-box">
        <div class="chosed">
          已选择
          <span>{{totalCheckedCart}}</span>件商品
        </div>
        <div class="sumprice">
          <em>总价（不含运费） ：</em>
...
export default {
  ...
  methods: {
    ...
    updateChecked(cart, event) {
      ...
      this.$store
        .dispatch('updateCheckedById', { skuId: cart.skuId, isChecked: isChecked })
        .then(res => {
          this.getData()
        })
        .catch(err => {
    ...
    deleteCartById(cart) {
      this.$store
        .dispatch('deleteCartById', cart.skuId)
        .then(res => this.getData())
        .catch(err => alert(err.message))
    },
  ...
  computed: {
    ...
    totalPrice() {
      ...
      this.cartInfoList.forEach(item => {
        item.isChecked ? (sum += item.skuNum * item.skuPrice) : ''
      })
      return sum
    },
    totalCheckedCart() {
